package io.dating.dating_ai_backend.profiles;
...
import static io.dating.dating_ai_backend.Utils.generateMyersBriggsTypes;
import static io.dating.dating_ai_backend.Utils.selfieTypes;
...
public class ProfileCreationService {
    ...
    private Profile generateProfileImage(Profile profile) {
        ...
        // Define constants for the prompt and negative prompt
        ...
        // Construct the JSON string using String.format
        final String jsonString = String.format(
                "{ \"prompt\": \"%s\", \"negative_prompt\": \"%s\", \"steps\": 40 }",
                prompt,
                negativePrompt);

        // Print status message with profile details
        System.out.println(String.format("Creating image for %s %s (%s)", profile.firstName(), profile.lastName(),
                profile.ethnicity()));
